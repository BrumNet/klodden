...
connectCatalog(price, minDeposit, material, color, style, gender) async {
  ...
  await FirebaseFirestore.instance.collection('uploads').doc(clothid).set({
...
connectFav() async {
  await FirebaseFirestore.instance.collection('Info').doc('Client').set({
...
uploadRedTail(id) async {
  await FirebaseFirestore.instance
      ...
      .set({'clothid': id, 'client': 'clientusername'}).catchError((e) {
    //s = 'Error';
  });
}
